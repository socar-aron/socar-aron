<!--
**socar-aron/socar-aron** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->

# 나는

어릴 때부터 스페이스 판타지 류의 세계관을 상상해 왔다. 간단히 설명할 때는 이것을 소설이라고 칭하고 있지만, 실제로는 위키 형식의 글 묶음이다. 소설적 묘사보다 세계에 대한 설명과 설정이 많다. 나는 다양한 분야에 관심이 있는데, 이 상상에 대한 완성도를 높이기 위해 여러 정보를 탐색하다 보니 그렇게 된 것이다. 대체로 과학 전반과 물리학, 천체물리학, 인지과학, 사회심리학, 미래학, 역사, 국제 정치, 경제 등에 관심이 있다.

## 프로그래밍/개발

2025년 12월 기준, 16년째 개발을 해왔다. 모바일 게임 3년 차, 모바일 메신저 6년 차, 카셰어링 서비스 8년 차이다. 주로 겪어 본 기술은 피처폰과 Android, iOS 환경이다. 코딩 자체에 관심이 있다기보다는, 내가 예상하는 미래상으로 인류가 이행해 가는 것을 재미있게 지켜보고 있으며, 그 과정에 소소한 기여를 하기를 원한다. 나의 이상은 너무나 멀어서 나의 인간적 수명 주기 내에 직접 달성하기는 힘들다고 보기 때문이다. 특히 바람직한 체계를 구축하고 그로부터 다수의 행복에 기여할 수 있는 선순환을 만드는 것을 원한다.

## 보유 기술

Kotlin, JAVA, Swift, Objective-C, C++, C#, Android, iOS, Cocos2d-x, Windows Forms, Git, SVN, Jira, Redmine, Slack.

## 글쓰기

2024-12-09 [반응형과 함수형과 논리식](writing/reactive%20functional%20and%20logical%20expressions.md)

2024-11-20 [쏘카프레임 앱 프레임워크와 개발자 경험](<writing/socarframe app framework and developer experience.md>)

2024-05-07 [리더십](https://docs.google.com/document/d/1JHnjd1kkykSuYL6eZmZGoCpBUfDVNpHhyPPZdDIrILM/edit?usp=sharing): 업무관, 팀의 규범과 문화, 대인 평가와 채용 등에 대한 생각들.

2024-03-27 [앱 개발 공통 지침의 의의](writing/the%20significance%20of%20common%20guidelines%20for%20app%20development.md)

2024-01-05 [코드 컨벤션은 어떤 것이어야 하는가](writing/code%20convention.md)

2022-12-15 [UI의 플랫폼 종속성](writing/platform%20dependency%20of%20ui.md)

2021-07-30 [팀장을 하면서 추구하고 있는 바에 대해](writing/things%20i%20pursue%20as%20a%20team%20leader.md)

2015-09-04 [프로그래밍을 배우는 게 유행인 요즘 드는 생각](writing/my%20thoughts%20these%20days%20when%20learning%20programming%20is%20trendy.md)

2010-08-03 [속도 경쟁에서 간결한 외관의 작용](writing/the%20power%20of%20a%20simple%20appearance%20in%20a%20speed%20race.md)
